...
<form id="search_form" action="#"
	...
	<div class="subfield">
      ...
      <div class="SwrapL">
					...
					<div class="left">
						<input type="checkbox" class="checkall" id="checkall" onclick="checkallbox(this)"  /> 全选 
							 ...
							 <input type="button" class="btns btn2" onclick="batchCollect()" title="收藏"> 
							  <input type="button"	class="btns btn3" title="分析"> 
				...
				<div class="lists" style="display: none;">
					<ul>
						...
						<c:if test="${idx.first==true}">
							<c:set var="docNo" value="${doc.docId}"></c:set>
						</c:if>
							<li>
								<div class="title">
									<h3>
										...
										<span class="T1"> <a id="ti_${idx.index}" val="${doc.docId }"  href="detail/detail.html?docNo=${doc.docId}&hlKwd='${params.kwd }'" target="detail_frame">${doc.TI}</a></span>
										<c:choose> 
									...
									<h4>
										<input type="button" class="btns btn1" onclick="download('${doc.docId}','${params.kwd}','1','${params.path}','${params.tName}')" title="下载"> 
										<input type="button" class="btns btn2" onclick="favorites('${doc.docId}','${idx.index }')" title="收藏">
										<%-- <input type="button" class="btns btn6" onclick="add2Compare('${doc.docId}','${idx.index }');" title="加入对比"> --%>
									</h4>
									<div class="clear"></div>
								</div>
								<div class="contents">
									...
									<div class="txts">
										<span class="T1" id="td_f_pa_${doc.AN}">申请(专利权人): ${doc.PA}</span> 
										<span class="T1" style="display: none;" id="td_f_au_${doc.AN}">发明设计人: <a href="javascript:;">${doc.AU}</a></span>
			...
			<!--End block5-->
			 ...
			 <div class="Rleft"> 
        <!--Begin block6-->
      	 <iframe id="detail_frame" name="detail_frame" src="search_detail.jsp"  frameborder="0" scrolling="auto" width="100%"  height="100%;" >
      	<%--   <div class="block6"> 
          <!--Begin items-->
          <div class="patent_detail items">
            <ul>
              <li class="cur"><a id="zlxm">著录项目</a></li>
              <li><a id="qlyqs">权利要求书</a></li>
            ...
            <h4>
              <input type="button" class="btns btn1" onclick="downPdf('${docNo}')" value="下载PDF">
              <input type="button" class="btns btn1b" onclick="view_pdf()" value="查看PDF">
              <!-- <input type="button" class="btns btn2" value="收藏"> -->
              <input type="button" class="btns btn3" onclick="print('col','div.block6')" value="打印">
            </h4>
            <div class="clear"></div>
          </div>
          <!--End items--> 
	...
	<form id="down_form" action="<%=path %>/detail/down.html" method="post" target="hideFrame">
        	<input type="hidden" name="pn" />
 	</form>

<iframe name="hideFrame" style="hidden">
</iframe>
...
$(function(){
	var docNo = '${docNo}';
	window.frames['detail_frame'].location.href="detail/detail.html?docNo="+docNo+"&hlKwd='${params.kwd}'";
})
...
function downPdf(id){
	//文件流
	var href = $('.items_list').contents().find('#pdf').val();
	$('#down_form')[0].docNo.value=href;
	$('#down_form').submit();
}
