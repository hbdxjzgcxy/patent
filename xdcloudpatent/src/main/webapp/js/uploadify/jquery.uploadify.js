...
(function($) {
	...
	var handlers = {
		...
		onSelect : function(file) {
			...
			if (settings.itemTemplate == false) {
				settings.itemTemplate = '<div id="${fileID}" class="uploadify-queue-item">\
					<div class="cancel">\
						<a title="关闭" href="javascript:void(0)">${fileName} (${fileSize}) <span class="data"></span></a>\
					</div>\
		...
		onSelectError : function(file, errorCode, errorMsg) {
			...
			if ($.inArray('onSelectError', settings.overrideEvents) < 0) {
				...
				case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
					this.queueData.errorMsg += '\n文件 "' + file.name
							+ '" 不是一个可接受的文件类型 (' + settings.fileTypeDesc + ').';
					break;
				}
			}
		...
		onUploadComplete : function(file) {
			...
			if ($.inArray('onUploadComplete', settings.overrideEvents) < 0) {
				$("div[id^='SWFUpload_']").not('#' + file.id).each(function() {
					$(this).remove();
				});
				...
				$
						.ajax({
							...
							success : function(data) {
								if (data == 1) {
									var overwrite = confirm('文件 "' + file.name
											+ '" 已经存在队列中.\nDo 你想替换现有的项队列吗?');
									if (!overwrite) {
										this.cancelUpload(file.id);
										$('#' + file.id).remove();
	...
	$.fn.uploadify = function(method) {
		...
		if (methods[method]) {
			...
			$.error('在 $.uploadify中方法  ' + method + ' 不存在 ');
		}
